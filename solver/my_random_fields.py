...
if __name__ == '__main__':
    ...
    parser.add_argument('-n','--num_points', type=int, default=128, help='Number of points')
    parser.add_argument('-L','--length', type=float, default=1.0, help='Length of domain')
    parser.add_argument('-l', '--length_scale', type=float, default=0.1, help='Scale of typical spacial deviations')
    parser.add_argument('-s', '--sigma', type=float, default=1.0, help='Amplitude of deviations')
    parser.add_argument('--nu', type=float, default=None, help='Smoothness parameter. Set to None for RBF Kernel')
    parser.add_argument('--mean', type=float, default=None, help='Mean value for distribution')
    parser.add_argument('-b', '--boundary_condition', type=str, default="periodic", help='Boundary condition type (periodic is the only one that has been tested)')
    parser.add_argument('-f', '--file', type=str, default='', help='Base filename to save data')
    ...
    grf = GRF_Mattern(dim=args.dim, size=args.num_points, length=args.length, nu=args.nu, l=args.length_scale, sigma=args.sigma, boundary=args.boundary_condition, constant_eig=args.mean, device=device)
    U = grf.sample(N)
